...
We start by reading a single dlf file and inspect the contents. We use the Soil chemical log that is bundled with `daisytools`.
...
Daisy spawn produces a directory hierarchy where each directory contains the same log files. We use the Daisy spawn outputs that are bundled with `daisytools`.
```{r}
data_dir <- system.file("extdata", package="daisytools")
...
We use the tracer and field nitrogen logs that are bundled with `daisytools` and read them with `read_dlf_dir`
```{r}
data_dir <- system.file("extdata", package="daisytools")
